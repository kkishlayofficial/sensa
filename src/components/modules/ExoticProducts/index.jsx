      ...
      <Container>
        <div className="exotic-products-section">
          ...
          <div className="products-list">
            <Slider {...settings}>
              {ProductList.filter(item => item.exotic).filter(item=> item.image).map(item => {
                return (
                  <Card img={item.image} title={item.name} subTitle={ item.category } height={'200px'} width={100} type='white' className='categoryCard' />
                )
              })}
            </Slider>
          </div>
